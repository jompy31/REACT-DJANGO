...
const Files = () => {
  ...
  const NotallowedFileNames = ['Background Header', 'Video Summary', 'CPT3', 'CPT3XL', 'Compact', 'Tabletop', 'PLD_3D', '3Dpads','PCB', 'MultiScale', 'Sensor', 'Fringes', 'Chips'];
  ...
  const filteredFiles = files.filter(file =>
    ...
    !NotallowedFileNames.includes(file.name)  // Exclude NotallowedFileNames
  );
    ...
    <Container class="full-width-container">
      ...
      {currentUser && currentUser.staff_status === 'administrator' && (
        <>
          ...
          {/* New section for additional files with custom names */}
