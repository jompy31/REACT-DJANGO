...
const ManageBlog = () => {
  ...
  const fetchBlogPosts = (token) => {
    TodoDataService.getAllBlogPosts(token)
      .then(response => {
        const modifiedBlogPosts = response.data.map(blogPost => ({
          ...blogPost,
          comment: '',
    ...
    <div style={{ width: '100%', minHeight: '100vh', overflow: 'auto', padding: '20px' }}>
      ...
      {user === null && (
        <Alert variant="warning">
          You are not logged in. Please <Link to={'/login'}>login</Link> to see our blog.
        </Alert>
      )}
      {user !== null && (
